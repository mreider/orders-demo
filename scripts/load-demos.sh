#!/usr/bin/env bash
# Load the presentation's demo notebooks into a Dynatrace tenant and
# make each one environment-shared with isPrivate=false. Works with any
# upstream dtctl version. Once dtctl PR #165 is merged and released,
# `dtctl apply -f <file> --share-environment --write-id` alone replaces
# this wrapper.
#
# Per-notebook steps:
...
if [ "$count" -eq 0 ]; then
  rm -f "$FILES_TMP"
  echo "no notebook YAMLs found under presentation/ or notebooks/"
  exit 1
fi
