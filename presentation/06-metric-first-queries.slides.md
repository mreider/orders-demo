...
**Skip the entity table. Shorter, cheaper, more stable DQL.**

---

# The point

With SDv2, `dt.service.name` and `service.name` are first-class metric dimensions. `filter` and `by` them on the metric directly.

- **Shorter queries.** No `fetch dt.entity.service` on the critical path.
- **Fewer scanned records.** Often an order of magnitude less.
- **Stable identity.** Dimension values ride through entity re-detection.

---

# Entity-joined vs. metric-first

Entity-joined (five lines):

```
timeseries cnt = sum(dt.service.request.count),
by: {dt.entity.service, endpoint.name}, from: now()-30m
...
Metric-first (two lines, same result):

```
timeseries cnt = sum(dt.service.request.count), by: {endpoint.name},
...
Watch **Scanned records** and **Execution time** in the cell metadata. Cheaper queries mean cheaper dashboards. Classic caveat: SDv1 services may not have `dt.service.name` populated; the service.name workaround fixes that.
